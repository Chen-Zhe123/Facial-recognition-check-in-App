...
public class Camera1Control implements ICameraControl {
    public static final String INIT_FACESDK = "init_facesdk";
...
//                    Log.modify_group("wtf", "onPreviewFrame-->");
...
//            });
            camera.setPreviewCallback(new Camera.PreviewCallback() {
                @Override
                public void onPreviewFrame(byte[] data, Camera camera) {
                    if (PreferencesUtil.getBoolean(INIT_FACESDK, false))
                        onFrameListener.onPreviewFrame(data, temp, size.width, size.height);
                }
            });
...
//        setFlashMode(FLASH_MODE_OFF);
    ...
    //                        }
    //                    });
    //                } catch (RuntimeException group_manager1) {
    //                    group_manager1.printStackTrace();
    //                    camera.startPreview();
    //                    takingPicture.set(false);
    //                }
    //            }
    ...
    private void opPreviewSize(int width, @SuppressWarnings("unused") int height) {
        ...
        if (camera != null && width > 0) {
            try {
                ...
                // Log.modify_group("wtf", "opPreviewSize-> " + optSize.width + " " +  optSize.height);
